...
function LunarSolarConverter() {
    ...
    this.SolarToLunar = function (solar) {
		...
		function convertToLunar(Day,Month,Year)
		{
			var DL=new Solar();
			DL.solarDay = parseInt(Day);
			DL.solarMonth = parseInt(Month);
			DL.solarYear = parseInt(Year);
			var obj=new LunarSolarConverter();
			var AL=obj.SolarToLunar(DL);
			return [AL.lunarDay,AL.lunarMonth,AL.lunarYear];
		}
		var mnths = { 
		...
		var Day=temp1[2];
		var Month=mnths[temp1[1]];
		var Year=temp1[3];
		var DAYW=['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];
		function time(){
		   var now=new Date();
		   ...
		   var lunar=convertToLunar(Day,Month,Year);
			$('#timeToday').html('Dương lịch Today: <span id="solar">'+Wday+', '+Day+'/'+Month+'/'+Year+'</span>');
			var lunar=convertToLunar(Day,Month,Year);
			$('#LunartimeToday').html('Âm lịch Today: <span id="lunar">'+lunar[0]+'/'+lunar[1]+'/'+lunar[2]+'</span>');	
		   $('#clock').html('<span class="date">&nbsp;&nbsp;'+nowTime+'</span>');
		...
		{
			return new Date(d,m,y).valueOf() ==new Date(Day,Month,Year).valueOf() ;
		}
		function contentCell(d,arr)
		{
				return '<span id="DL"><b>'+d+'</b></span><br><span id="AL">&nbsp;&nbsp;'+arr[0]+'/'+arr[1]+'</span>';
		}
		//tao lich
		...
		{	var html='';
			...
			var fistDayOfMonth =new Date(thisYear,thisMonth-1,1);
			var temp2=fistDayOfMonth.toString().split(" ");
			...
			var i,j; var k=1;
			var rownum=5;
			if(daysInMonth(thisMonth,thisYear)==30&&index==7)
				rownum=6;
			else if(daysInMonth(thisMonth,thisYear)==31&&(index==7||index==7))
				rownum=6;
